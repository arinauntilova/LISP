...
predicates
	nondeterm record(sname, phoneNum, address)
	nondeterm owns(sname, brand, color, price)
	nondeterm hasBankDeposit(sname, bank, account, amount)

	% Rules
	nondeterm viewCar(phoneNum, sname, brand, price)
	nondeterm brandOnly(phoneNum, brand)
	nondeterm viewData(sname, city, street, bank, phoneNum)
...
clauses
	...
	% Rules
	viewCar(PhoneNum, Sname, Brand, Price) :- record(Sname, PhoneNum, _),
									 owns(Sname, Brand, _, Price).

	brandOnly(PhoneNum, Brand) :- viewCar(PhoneNum, _, Brand, _).

	viewData(Sname, City, Street, Bank, PhoneNum) :- record(Sname, PhoneNum, address(City, Street, _, _)),
